...
class UserSummary(object):
    ...
    def main(self):
        ...
        data = tagged_user_blog.rdd.map(preprocess_data).reduceByKey(lambda x, y: (x + y)).map(
        ...
        pd.DataFrame(result_dict, index=None).to_csv(
            "/home/spark/hxkTest/spark_script/weibo_user_summary/untagged_approved_user_keyword.csv", encoding="utf-8")
