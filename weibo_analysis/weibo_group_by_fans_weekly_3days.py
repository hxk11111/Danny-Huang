...
class GroupByFans(object):
    ...
    def read_distinct(self, dataframe):
        path = []
        for each_day in self.days_list:
            path.append(self.path % each_day)
    ...
    def main(self):
        ...
        paired_rdd = rdd.map(lambda x: (
            x["uid"].strip().replace("%0A", ""), ([x["publish_time"]], int(x["fans"]),
                       0 if ((x["praise"] == u"\u8d5e") or (x["praise"] is None) or (x["praise"] == "")) else int(
                           x["praise"] if x["praise"].strip() != "" else 0),
                       0 if (x["repeat"] == u"\u8bc4\u8bba" or x["repeat"] is None) else int(
                           x["repeat"] if x["repeat"].strip() != "" else 0),
                       0 if (x["forward"] == u"\u8f6c\u53d1" or x["forward"] is None) else int(
                           x["forward"] if x["forward"].strip() != "" else 0))))\
            .reduceByKey(info_parse).map(add_group_tag) \
        ...
        for each in paired_rdd:
            ...
            total_praise.append(each[1][1])
            total_repeat.append(each[1][2])
            total_forward.append(each[1][3])
        ...
        user_total_by_fans = []
        fans_num_rdd = fans_rdd.map(lambda x: (x["uid"].strip().replace("%0A", ""), int(x["fans"]))).reduceByKey(lambda x,y: x).map(group_tag_for_all).reduceByKey(lambda x, y: x + y).sortByKey().collect()
        ...
        fans_num_df = pd.DataFrame(
            {
                ...
                "z total likes": total_praise,
                "z total comments": total_repeat,
                "z total retweets": total_forward
            }
        )
        fans_num_df.to_csv(self.output + "fans_num_FROM_%s_TO_%s.csv" % (self.days_list[0], self.days_list[-1]),
                           index=None)
