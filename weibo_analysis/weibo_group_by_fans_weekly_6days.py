...
class GroupByFans(object):
    def __init__(self):
        ...
        hdfs_end_time = (datetime.datetime.strptime(hdfs_start_time, "%Y-%m-%d") + timedelta(days=5)).strftime(
            "%Y-%m-%d")
    ...
    def main(self):
        ...
        paired_rdd = rdd.map(lambda x: (
            x["uid"], ([x["publish_time"]], int(x["fans"]),
                       0 if ((x["praise"] == u"\u8d5e") or (x["praise"] is None) or (x["praise"] == '')) else int(x["praise"] if x["praise"] != "" else 0),
                       0 if ((x["repeat"] == u"\u8bc4\u8bba") or (x["repeat"] is None) or (x["repeat"] == '')) else int(x["repeat"]),
                       0 if ((x["forward"] == u"\u8f6c\u53d1") or (x["forward"] is None) or (x["forward"] == '')) else int(
        ...
        fans_num_df = pd.DataFrame(
            {
                ...
                "z total praises": total_praise,
                "z total repeats": total_repeat,
                "z total forwards": total_forward
            }
        )
